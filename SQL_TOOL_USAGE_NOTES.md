...
#### ⚠️ 功能受限模型
這些模型不支援 Function Calling，**不建議用於 SQL 查詢**：
...
- 無法執行實際的資料查詢
- 只能提供一般性的 SQL 語法建議
- **建議**: 改用 Claude 3 Haiku (快速且經濟) 或 GPT-4o-mini 進行 SQL 查詢

**為什麼會失敗**:
當您問「最近的5筆訂單是什麼？」時，這些模型會：
1. 嘗試列出所有資料表（包括系統內部表，已修正為只顯示業務表）
2. 無法執行實際的 SELECT 查詢
3. 只能提供 SQL 語法說明

**解決方案**: 切換到支援 Function Calling 的模型（Claude 或 GPT-4 系列）
