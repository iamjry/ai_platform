        ...
        if os.path.exists(prompts_path):
            with open(prompts_path, 'r', encoding='utf-8') as f:
                ...
                if data and 'agent_prompts' in data:
                    logger.info("✓ Loaded agent prompts from config file")
                    return data['agent_prompts']
        logger.warning(f"Config file not found at {prompts_path}, using default prompts")
        return default_prompts
    except Exception as e:
        logger.error(f"Error loading agent prompts: {e}, using default prompts")
        return default_prompts
...
def detect_tool_intent(task: str) -> Optional[tuple]:
    ...
    sql_keywords = [
        "查詢資料庫", "查询资料库", "查询数据库", "資料庫查詢", "数据库查询",
        "訂單", "订单", "客戶", "客户", "產品", "产品", "庫存", "库存",
        "銷售", "销售", "生產", "生产", "出貨", "出货", "工單", "工单",
        "查看.*資料", "查看.*数据", "顯示.*資料", "显示.*数据",
        "有哪些", "多少筆", "多少笔", "統計", "统计", "總額", "总额",
        "最近.*訂單", "最近.*订单", "進行中", "进行中", "待處理", "待处理",
        "customers", "products", "orders", "inventory", "sales"
    ]

    has_sql_keyword = any(re.search(keyword, task_lower) for keyword in sql_keywords)

    if has_sql_keyword:
        # First, check if we need to list tables
        if any(keyword in task_lower for keyword in ["有哪些表", "有哪些資料表", "資料庫結構", "数据库结构", "list tables"]):
            return ("sql_list_tables", {})

        # Then check for schema queries
        schema_patterns = [
            r"(customers|products|sales_orders|order_items|production_orders|inventory_transactions|shipments).*結構",
            r"(customers|products|sales_orders|order_items|production_orders|inventory_transactions|shipments).*schema",
            r".*表.*欄位", r".*表.*字段"
        ]
    ...
    try:
        ...
        if tool_intent and request.model not in function_calling_models:
            ...
            try:
                ...
                if tool_name == "send_email":
                ...
                elif tool_name == "sql_list_tables":
                    # Format table list
                    ...
                    if tables:
                        ...
                        for t in tables:
                            ...
                            table_items.append(f"• **{name}** ({rows} rows, {cols} columns)\n  {desc}")
                        result = f"✅ 資料庫結構\n\n找到 {len(tables)} 個資料表：\n\n" + "\n\n".join(table_items)
                    else:
                        result = "✅ 資料庫中沒有找到資料表。"
                elif tool_name == "sql_get_schema":
                    ...
                    if columns:
                        col_items = []
                        for c in columns:
                            col_name = c.get('name', 'N/A')
                            col_type = c.get('type', 'N/A')
                            nullable = "NULL" if c.get('nullable', True) else "NOT NULL"
                            constraint = c.get('constraint', '')
                            constraint_str = f" [{constraint}]" if constraint else ""
                            col_items.append(f"• {col_name}: {col_type} {nullable}{constraint_str}")
                        result = f"✅ 資料表結構: **{table_name}**\n\n欄位 ({len(columns)} columns):\n" + "\n".join(col_items)
                    else:
                        result = f"✅ 資料表 '{table_name}' 的結構資訊為空。"
                elif tool_name == "sql_query":
                    # Format query results
                    if tool_result.get('success'):
                        rows_returned = tool_result.get('rows_returned', 0)
                        query_results = tool_result.get('results', [])
                        query_text = tool_result.get('query', '')
                        ...
                        if query_results:
                            ...
                            if len(query_results) > 0:
                                ...
                                # Create rows (limit to 20 for readability)
                                display_rows = query_results[:20]
                                rows = []
                                ...
                                more_info = f"\n\n(顯示 {len(display_rows)} / {rows_returned} 筆資料)" if rows_returned > 20 else f"\n\n(共 {rows_returned} 筆資料)"

                                result = f"✅ SQL 查詢結果\n\n```\n{table_str}\n```{more_info}"
                            else:
                                result = "✅ SQL 查詢成功，但沒有返回資料。"
                        else:
                            result = "✅ SQL 查詢成功，但沒有返回資料。"
                    else:
                        error_msg = tool_result.get('error', '未知錯誤')
                        result = f"❌ SQL 查詢失敗\n\n錯誤: {error_msg}"
                elif tool_name == "sql_explain_query":
                    # Format EXPLAIN results
                    if tool_result.get('success'):
                        plan = tool_result.get('execution_plan', [])
        ...
        # Load agent prompts from config file
