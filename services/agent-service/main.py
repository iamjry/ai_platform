...
def detect_tool_intent(task: str) -> Optional[tuple]:
    ...
    if has_sql_keyword:
        ...
        # Default to sql_query for general database questions
        # Let the LLM generate the actual SQL query based on the question
        logger.info(f"SQL query pattern detected - Question: '{task}'")
        return ("sql_query", {
            "query": f"-- Question: {task}\n-- This will be generated by the LLM"
        })
